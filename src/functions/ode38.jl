...
# f!(dx, t, x, p)
@inline function ode38(f!, x0, tspan, p; nSteps=100, K1=similar(x0), K2=similar(x0), K3=similar(x0), K4=similar(x0), K5=similar(x0))
    # 3/8 Runge-Kutta Method
    # http://www.mymathlib.com/diffeq/runge-kutta/runge_kutta_3_8.html
    x = copy(x0)
    return ode38!(x, f!, x0, tspan, p, K1, K2, K3, K4, K5; nSteps=nSteps)
end

@inline function ode38!(x, f!, x0, tspan, p, K1, K2, K3, K4, K5; nSteps=100)
    # 3/8 Runge-Kutta Method
    # http://www.mymathlib.com/diffeq/runge-kutta/runge_kutta_3_8.html
    x .= x0
    t = tspan[1]
